...
class ProfileService {
  ...
  async updateCustomer(customerId: number, data: Partial<Customer>): Promise<Customer> {
    try {
      ...
      const response = await axiosClient.patch(`/customers/${customerId}`, data);
      return response as unknown as Customer;
    } catch (error) {
    ...
    try {
      ...
      const convertToArray = (data: any): any[] => {
        ...
        if (typeof data === 'object') {
          ...
          const filtered = values.filter((item: any) => 
            item && typeof item === 'object' && item.order_id
          );
      ...
      const orders: Order[] = ordersArray.map((item: any) => ({
        ...
        order_details: (item.order_details || []).map((detail: any) => ({
          ...
          name: detail.name || (detail.product?.name) || (detail.service?.name) || 'Sản phẩm',
  ...
  async createPet(customerId: number, petData: Partial<Pet>): Promise<Pet> {
    try {
      const response = await axiosClient.post(`/pets/customer/${customerId}`, petData);
