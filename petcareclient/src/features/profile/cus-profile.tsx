...
import type { Customer, Pet, Order, OrderDetail } from "@/lib/profileService";
...
export default function CustomerProfilePage() {
  ...
  const fetchCustomerData = async (phone: string) => {
    try {
      ...
      if (customerResponse?.customer_id) {
        ...
        try {
          const petsResponse = await profileService.getPetsByCustomer(customerResponse.customer_id);
        ...
        try {
          const ordersResponse = await profileService.getOrdersByCustomer(customerResponse.customer_id);
  ...
  const handleSaveProfile = async () => {
    try {
      ...
      const updated = await profileService.updateCustomer(customer.customer_id, editData);
      setCustomer(updated);
      setIsEditing(false);
      ...
      <div className="min-h-screen bg-gradient-to-br from-orange-50 to-gray-100">
        ...
        <div className="bg-white border-b shadow-sm">
          <div className="max-w-7xl mx-auto px-6 py-4">
            <h1 className="text-2xl font-bold text-gray-800">Tra cứu khách hàng</h1>
          </div>
        </div>
        ...
        <div className="flex items-center justify-center min-h-[80vh] p-4">
          <div className="bg-white rounded-2xl shadow-xl max-w-md w-full p-8">
            <div className="text-center mb-6">
              ...
              <h2 className="text-2xl font-bold text-gray-800">Tra cứu khách hàng</h2>
              <p className="text-gray-500 mt-2">Nhập số điện thoại để xem thông tin khách hàng</p>
            </div>
                  ...
                  {isEditing ? (
                    ...
                    <>
                      ...
                      <div>
                        ...
                        <p className="font-medium text-gray-700 text-sm">{customer.address || "-"}</p>
                      </div>
                      <div>
                        <p className="text-xs text-gray-500 uppercase">Ngày tham gia</p>
                        <p className="font-medium text-gray-800">{formatDate(customer.created_at)}</p>
                      </div>
                      <div>
                        <p className="text-xs text-gray-500 uppercase">Email</p>
                        <p className="font-medium text-gray-700 text-sm">{customer.email || "-"}</p>
                      </div>
                      <div className="pt-4 border-t mt-2">
                        ...
                        <div className="flex justify-between items-center mt-2">
                          ...
                          <p className="text-lg font-semibold text-gray-800">{totalOrders}</p>
                        </div>
                      </div>
                    </>
                  )}
          ...
          <div className="lg:col-span-2 space-y-6">
            ...
            <div className="bg-white rounded-2xl shadow-lg overflow-hidden">
              <div className="flex justify-between items-center p-6 border-b">
                ...
                <button 
                  onClick={() => navigate(`/pets/create?customerId=${customer.customer_id}`)}
                      ...
                      >
                        <div className="flex items-start gap-3">
                          ...
                          <div className="flex-1">
                            <h4 className="font-semibold text-gray-800">{pet.pet_name}</h4>
                            <p className="text-sm text-gray-600">{pet.breed || "Không rõ giống"}</p>
                            {pet.weight && <p className="text-xs text-gray-500 mt-1">⚖️ {pet.weight} kg</p>}
                            {pet.age && <p className="text-xs text-gray-500">🎂 {pet.age} tuổi</p>}
                          </div>
                        </div>
                      </div>
                    ))}
            ...
            <div className="bg-white rounded-2xl shadow-lg overflow-hidden">
              ...
              {ordersError && orders.length === 0 ? (
                <div className="text-center py-12">
                  <p className="text-gray-500">Không thể tải lịch sử giao dịch</p>
                  ...
                  <table className="w-full">
                    <thead className="bg-gray-50">
                      <tr>
                        ...
                        <th className="text-left p-4 text-sm font-medium text-gray-600">DỊCH VỤ/SẢN PHẨM</th>
                        <th className="text-left p-4 text-sm font-medium text-gray-600">TỔNG TIỀN</th>
                      ...
                      {filteredOrders.map((order) => (
                        ...
                        >
                              ...
                              <div className="space-y-1">
                                {order.order_details.map((detail) => (
                                  <div key={detail.id} className="text-sm">
                                    ...
                                    <span className="text-gray-500 text-xs ml-2">x{detail.quantity}</span>
                                    {detail.pet_name && (
                                      <span className="text-xs text-gray-400 ml-2">(Cho {detail.pet_name})</span>
                                    )}
                                  </div>
                                ))}
                              </div>
                          ...
                          <td className="p-4 text-center">
                            <span className={`inline-block px-3 py-1 rounded-full text-xs font-medium ${getStatusColor(order.status).color}`}>
