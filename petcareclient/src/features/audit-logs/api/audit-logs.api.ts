...
export const getRefInfo = (entry: HistoryEntry) => {
    ...
    const info = typeMap[entry.entity_type] || {
...
export const getAuditLogs = async (params?: {
    ...
    const response = await apiClient.get<HistoryEntry[]>("/stores/activity", {
        params,
    });
    ...
    const uniqueData = Array.from(
        new Map(
            data.map((item) => [
                `${item.entity_type}-${item.id}-${item.created_at}`,
                item,
            ]),
        ).values(),
    );
...
export const getRoleHistory = async (
    ...
    const response = await apiClient.get<HistoryEntry[]>(
        `/stores/${storeId}/roles/${roleId}/history`,
    );
    return Array.isArray(response.data) ? response.data : [];
};
