...
export class MailService {
  ...
  async sendInvitationEmail(
    invitationData: InviteStaffResponseDto,
    full_name?: string,
  ) {
    try {
      ...
      const invitationLink = buildInvitationUrl(
        frontendUrl,
        invitationData.invitation.token,
      );
      const formattedExpiryDate = new Date(
        invitationData.invitation.expires_at,
...
Đây là email tự động. Vui lòng không trả lời.`;
      ...
      this.logger.log(
        `Invitation email sent successfully to: ${invitationData.invitation.email}`,
      );
    } catch (error) {
